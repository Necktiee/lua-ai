  ...
  try {
    const suggestion = await chat({
      messages: [
        {
          role: "system",
          content: "เป็นแจ๋ว เลขาส่วนตัว. จากข้อมูลวันนี้ แนะนำ 2-3 ข้อสั้นๆ ภาษาไทย (เช่น 'โทรหาแม่ก่อน 9 โมง', 'ออกบ้านเร็วขึ้นเพราะฝน'). ตอบสั้น ไม่เกิน 3 บรรทัด ขึ้นต้นแต่ละบรรทัดด้วย '- '.",
        },
  ...
  try {
    ...
    const suggestion = await chat({
      messages: [
        {
          role: "system",
          content: "เป็นแจ๋ว เลขาส่วนตัว. แนะนำลำดับความสำคัญ 3 อย่างแรกที่ควรทำพรุ่งนี้ เรียงตามความสำคัญ. ภาษาไทย สั้นกระชับ. ขึ้นต้นด้วย 1. 2. 3.",
        },
