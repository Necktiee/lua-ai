/**
 * Nudge cron — split from overloaded daily route.
 * Sends follow-up nudges + overdue todo alerts at user's local 09:00.
 */
...
export async function GET(req: Request) {
  ...
  const rows = await getUsersAtLocalTime(9, 0);
  const nudgeUsers = filterAllowed(rows.map((r) => r.userId));
  const nudgeTz = new Map(rows.map((r) => [r.userId, r.timezone]));
  let nudged = 0;
  let todoNudged = 0;
  ...
  return Response.json({ nudged, overdueTodoNudged: todoNudged });
}
