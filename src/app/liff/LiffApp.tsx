...
type Status = "loading" | "logging-in" | "verifying" | "ready" | "unauthorized" | "error";
...
export default function LiffApp({ liffId }: { liffId: string | null }) {
  ...
  useEffect(() => {
    ...
    async function run() {
      if (!liffId) {
        setError("ยังไม่ตั้งค่า LIFF ID");
        setStatus("error");
        return;
      }
      ...
      try {
        ...
        const data = (await res.json().catch(() => ({}))) as { error?: string; userId?: string };
        if (cancelled) return;

        if (res.status === 401 || res.status === 403) {
          setError("บัญชี LINE นี้ยังไม่มีสิทธิ์ใช้ Dashboard");
    ...
    <main className="min-h-[100dvh] flex flex-col items-center justify-center bg-zinc-50 dark:bg-zinc-950 p-6 font-sans">
      <div className="max-w-sm w-full bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800 p-8 space-y-5 text-center">
        <div className="flex items-center justify-center gap-2">
          ...
          <h1 className="text-lg font-semibold text-zinc-900 dark:text-zinc-50 tracking-tight">แจ๋ว Dashboard</h1>
        </div>
        ...
        {(status === "error" || status === "unauthorized") && (
          <div className="space-y-3">
            ...
            <button
              onClick={() => {
                if (status === "unauthorized") window.location.href = "https://line.me/R/";
            ...
            >
              ...
              {status === "unauthorized" ? "กลับไปที่ LINE" : "ลองใหม่"}
            </button>
          </div>
        )}
      </div>
